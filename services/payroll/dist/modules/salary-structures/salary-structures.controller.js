"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.SalaryStructuresController = void 0;
const common_1 = require("@nestjs/common");
const swagger_1 = require("@nestjs/swagger");
const shared_1 = require("@multi-tenant/shared");
const salary_structures_service_1 = require("./salary-structures.service");
const create_salary_structure_dto_1 = require("./dto/create-salary-structure.dto");
let SalaryStructuresController = class SalaryStructuresController {
    constructor(salaryStructuresService) {
        this.salaryStructuresService = salaryStructuresService;
    }
    create(orgId, dto) {
        return this.salaryStructuresService.create(orgId, dto);
    }
    findCurrentByUser(userId, orgId) {
        return this.salaryStructuresService.findCurrentByUser(orgId, userId);
    }
    findHistoryByUser(userId, orgId) {
        return this.salaryStructuresService.findHistoryByUser(orgId, userId);
    }
};
exports.SalaryStructuresController = SalaryStructuresController;
__decorate([
    (0, common_1.Post)(),
    (0, common_1.HttpCode)(common_1.HttpStatus.CREATED),
    (0, swagger_1.ApiCreatedResponse)({ description: 'Salary structure created' }),
    __param(0, (0, shared_1.OrgId)()),
    __param(1, (0, common_1.Body)()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [String, create_salary_structure_dto_1.CreateSalaryStructureDto]),
    __metadata("design:returntype", void 0)
], SalaryStructuresController.prototype, "create", null);
__decorate([
    (0, common_1.Get)(':userId'),
    (0, common_1.HttpCode)(common_1.HttpStatus.OK),
    (0, swagger_1.ApiParam)({ name: 'userId', type: String }),
    (0, swagger_1.ApiOkResponse)({ description: 'Get current salary structure for user' }),
    __param(0, (0, common_1.Param)('userId', common_1.ParseUUIDPipe)),
    __param(1, (0, shared_1.OrgId)()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [String, String]),
    __metadata("design:returntype", void 0)
], SalaryStructuresController.prototype, "findCurrentByUser", null);
__decorate([
    (0, common_1.Get)(':userId/history'),
    (0, common_1.HttpCode)(common_1.HttpStatus.OK),
    (0, swagger_1.ApiParam)({ name: 'userId', type: String }),
    (0, swagger_1.ApiOkResponse)({ description: 'Get salary structure history for user' }),
    __param(0, (0, common_1.Param)('userId', common_1.ParseUUIDPipe)),
    __param(1, (0, shared_1.OrgId)()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [String, String]),
    __metadata("design:returntype", void 0)
], SalaryStructuresController.prototype, "findHistoryByUser", null);
exports.SalaryStructuresController = SalaryStructuresController = __decorate([
    (0, swagger_1.ApiTags)('Salary Structures'),
    (0, swagger_1.ApiBearerAuth)(),
    (0, common_1.UseGuards)(shared_1.TenantIsolationGuard, shared_1.RolesGuard, shared_1.PermissionsGuard),
    (0, common_1.Controller)({ path: 'salary-structures', version: '1' }),
    __metadata("design:paramtypes", [salary_structures_service_1.SalaryStructuresService])
], SalaryStructuresController);
//# sourceMappingURL=salary-structures.controller.js.map